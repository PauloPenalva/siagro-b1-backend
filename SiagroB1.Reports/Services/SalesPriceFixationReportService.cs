...
/// <summary>
/// Gera o "espelho de fixação de preço" de VENDA: comprovante enviado ao cliente
/// confirmando o preço fixado para uma parcela de um contrato de venda a fixar (PAF).
/// Espelha <see cref="PriceFixationReportService"/> (compra).
/// </summary>
public class SalesPriceFixationReportService(
    IUnitOfWork db,
    ...
    ReportHeaderService reportHeader)
{
    public async Task<byte[]> GeneratePdfAsync(Guid key)
    {
        ...
        var pdfExport = new PDFSimpleExport { ShowProgress = false, Title = "EspelhoFixacaoVenda" };
    ...
    {
        var fixation = await db.Context.SalesContractsPriceFixations
            .AsNoTracking()
        ...
        // aprovação, rejeitada ou estornada mandaria ao cliente um documento sobre um
        // preço que não vale.
