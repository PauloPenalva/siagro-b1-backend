...
/// fila da diretoria (<see cref="PriceFixationStatus.InApproval"/>). Espelha
/// <c>PurchaseContractsPriceFixationsCancelService</c>.
/// </summary>
/// <remarks>
/// Não vira <see cref="PriceFixationStatus.Canceled"/>: o estorno existe para corrigir uma
/// aprovação, não para matar o registro. O volume continua reservado (InApproval reserva
/// igual a Confirmed); só o preço sai do total, porque TotalPrice conta apenas Confirmed.
/// Para devolver o volume ao saldo, exclui-se a fixação ou a diretoria a rejeita.
/// </remarks>
    ...
    FinancialDocumentsCancelService financialDocuments)
{
    public async Task ExecuteAsync(Guid fixationKey, string canceledBy)
    {
        ...
        await financialDocuments.EnqueueCancelByOriginAsync(
            FinancialDocumentOrigin.SalesContractPriceFixation,
            fixation.Key,
