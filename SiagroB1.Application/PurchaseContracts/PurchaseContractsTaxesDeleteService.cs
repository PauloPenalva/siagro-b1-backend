...
public class PurchaseContractsTaxesDeleteService(
    AppDbContext context, ILogger<PurchaseContractsTaxesDeleteService> logger)
{
    public async Task<bool> ExecuteAsync(Guid parentKey, Guid associationKey)
    {
        try
        {
            ...
            context.PurchaseContractsTaxes.Remove(existingEntity);
